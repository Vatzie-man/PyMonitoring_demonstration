...
class ModifyDB():
    ...
    def db_modification(self, data) -> None:
        ...
        def get_suffix(i):
            suffixes = ["Time", "Fan", "Compressor", "Pump", "Temp and Free", "Condensing Pressure", "Evaporating Pressure", "Saturation Temp",
                        "Super Heating", "Liquid Temp", "Sub-Cooling"]
            return suffixes[i - 1] if i <= len(suffixes) else "non"

        def get_alert(i):
            alerts = ["time", "pm", "non", "non", "line_after_cdu3", self.dict_for_db["ACH2"]["alert"], self.dict_for_db["ACH4"]["alert"],
                      self.dict_for_db["ACH3"]["alert"], self.dict_for_db["ACH1"]["alert"], "line_before_pcw1_h0", "non", "non", "non"]
            return alerts[i - 1] if i <= len(alerts) else "non"
        ...
        def get_title(i):
            descriptions = ["Time", "Ares", "CDU1", "CDU2", "CDU3", "ACH2", "ACH4", "ACH3", "ACH1", "PCW1 H0", self.PCW_H1, "Po UPS.1", "Po UPS-1"]
